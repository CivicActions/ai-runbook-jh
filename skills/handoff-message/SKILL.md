...
## Voice
Apply `.agents/style/voice.md`. Apply it to all generated text, handoff prose, next steps, risk notes. Run shared handoff prose through `tone-check` before publishing.
...
- **Invokes:** `lessons-learned` — run after the handoff file is saved, every session. Captures what went well, what didn't, and what to carry forward. Write to `.agents/lessons/` before closing.
